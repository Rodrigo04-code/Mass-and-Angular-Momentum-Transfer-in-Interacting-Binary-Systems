...
Description:
- Adiabatic Parker Wind Initialization for Idefix
- Implements a non-dimensional spherically symmetric wind
- Uses Newton-Raphson to solve the adiabatic Bernoulli equation for velocity
...
KOKKOS_INLINE_FUNCTION real get_cs2_adiabatic(real v, real r, real rs, real vs, real gam) {
    real rho_ratio = (rs*rs*vs) / (r*r*v);
    return vs*vs * pow(rho_ratio, gam - 1.0);
}

KOKKOS_INLINE_FUNCTION real fParkerAdiabatic(real v, real r, real rs, real vs, real gam) {
    real cs2 = get_cs2_adiabatic(v, r, rs, vs, gam);
    return 0.5*v*v + cs2/(gam-1.0) - (2.0/r) - 0.5;
}

KOKKOS_INLINE_FUNCTION real dfParkerAdiabatic(real v, real r, real rs, real vs, real gam) {
    real cs2 = get_cs2_adiabatic(v, r, rs, vs, gam);
    return v - (cs2 / v);
}
...
KOKKOS_INLINE_FUNCTION real solve_v_at_point(real r, real rs, real vs, real gam, real v_guess) {
    ...
    for(int it=0; it<50; it++) {
        real f = fParkerAdiabatic(v, r, rs, vs, gam);
        real df = dfParkerAdiabatic(v, r, rs, vs, gam);
...
void UserdefBoundary(Hydro *hydro, int dir, BoundarySide side, real t) {
    ...
    if(side == left) {
        ...
        idefix_for("BoundaryLeft", 0, data->np_tot[KDIR], 0, data->np_tot[JDIR], ibeg, iend,
            ...
            real cs2 = get_cs2_adiabatic(v_exact, r_ghost, rs, vs, gam);
    ...
    for(int k=0; k<d.np_tot[KDIR]; k++) {
        for(int j=0; j<d.np_tot[JDIR]; j++) {
            for(int i=0; i<n1; i++) {
                ...
                d.Vc(RHO,k,j,i) = rho;
                real cs2 = get_cs2_adiabatic(v_sol[i], r, rs, vs, gam);
